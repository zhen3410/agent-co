...
test('当前协作型活跃智能体会明确要求通过 invokeAgents 做结构化继续传播', () => {
  const filePath = path.join(__dirname, '..', '..', 'data', 'agents.json');
  const store = JSON.parse(fs.readFileSync(filePath, 'utf8'));
  const activeAgents = Array.isArray(store.activeAgents) ? store.activeAgents : [];
  const requiredAgents = ['CPO战略官', '创意猎手', '用户研究员', '增长黑客'];

  for (const name of requiredAgents) {
    const agent = activeAgents.find(item => item.name === name);
    assert.ok(agent, `missing agent: ${name}`);
    const prompt = String(agent.systemPrompt || '');
    assert.match(prompt, /invokeAgents/, `${name} should mention invokeAgents structured routing`);
    assert.match(prompt, /不要依赖正文.*@/, `${name} should forbid relying on plain @ routing`);
  }
});
