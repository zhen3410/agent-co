...
test('React 页面将内联事件处理函数挂载到 window，并在 load 后绑定 DOM', () => {
  ...
  assert.ok(html.includes('id="chatWorkdirSelect"'), 'should render workdir selector in chat page');
  assert.ok(html.includes("fetch('/api/workdirs/options'"), 'should load workdir options from server');
