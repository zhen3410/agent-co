...
| `bot-room-mcp-server.ts` | MCP 回调服务：`bot_room_post_message` 和 `bot_room_get_context` |
...
| `/api/callbacks/post-message` | POST | 智能体回调发消息（需 x-bot-room-callback-token） |
...
CLI 智能体的详细输出会记录到 `BOT_ROOM_VERBOSE_LOG_DIR`（默认 `logs/ai-cli-verbose`）：
